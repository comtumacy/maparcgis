<template>
    <div class="adminLeftUserEditAlert">
      ...
      <el-button class="button1" type="success" v-on:click="submitForm('ruleForm')">确定修改</el-button>
      <el-button class="button2" type="info" v-on:click="$emit('sent')">取消修改</el-button>
    </div>
</template>
...
export default {
  components: {},
  props: ['adminLeftUserEditAlertSign', 'row'],
  created () {
    this.getDepartment()
  },
  data () {
  ...
  computed: {},
  watch: {
    adminLeftUserEditAlertSign (val) {
      if (val === true) {
        this.enter()
      }
    }
  ...
  methods: {
    ...
    enter () {
      ...
      this.ruleForm.department = this.row.department
      this.ruleForm.gender = this.row.gender
      this.ruleForm.tel = this.row.tel
